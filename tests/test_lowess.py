...
def test_elimination_dataframe_sorted():
    df = redundant._get_elim_df_ordered(sample_elimination_dict(), multi=False, use_accuracy=False)
...
def test_lowess_outputs_shapes():
    ...
    x, y, z, xnew, ynew = redundant._cal_lowess(d, redundant.DEFAULT_FRAC, False, False)
    assert len(x) == len(y)
